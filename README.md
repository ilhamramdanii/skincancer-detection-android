...
> ⚠️ **Disclaimer:** aplikasi ini hanya untuk tujuan edukasi dan referensi. Selalu konsultasikan dengan dokter untuk diagnosis medis yang akurat.
...
3. Run ▶️ ke emulator atau HP via USB debugging (minSdk 24).
...
3. Pilih foto lesi kulit dari galeri — gambar tampil di `ImageView`.
4. Hasil analisis muncul otomatis di kolom **Hasil Deteksi**, contoh:
   ```
   malignant: 92.40%
   benign: 7.60%
   ```
   Jika keyakinan di bawah 1% per kelas, hasil disaring oleh `setScoreThreshold(0.001f)` dan maksimal 2 hasil (`setMaxResults(2)`).
5. Ulangi dengan foto lain bila perlu. Ingat: ini bukan diagnosis medis.

## 🧠 Info Model & Dataset

- Model aktif: `app/src/main/ml/model_7725_metadata(2).tflite` (~14 MB, dengan metadata preprocessing + label).
...
Proyek akademik (Tugas Akhir). Gunakan dan modifikasi secara bebas untuk keperluan edukasi.
